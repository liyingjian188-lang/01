...
- 不要提交密钥。服务端当前支持环境变量覆盖的仅有非敏感项：`VISION_MODELS`（CSV）、`SILICONFLOW_BASEURL`（默认 `https://api.siliconflow.cn`）、`TEMPLATE_PATH`。`siliconflow_api_key` 需要写在 `screensot-server/config.json` 中。
